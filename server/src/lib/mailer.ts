...
const usesPlaceholderSmtp =
  String(env.SMTP_HOST ?? "").includes("example.com") ||
  env.SMTP_USER === "username" ||
  env.SMTP_PASS === "password" ||
  String(env.SMTP_FROM ?? "").includes("graduateschool.local");

if (isMailerConfigured && !usesPlaceholderSmtp) {
