...
export default function TermSettings() {
  ...
  return <div className="space-y-5 animate-fade-up">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">Set the grade submission deadline for each semester. Faculty reminders and five-day coordinator escalations use this date.</p>
          </div>
        </div>
    ...
    {loading ? <Spinner label="Loading academic semesters..." /> : <Card className="overflow-hidden p-0"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead><tr className="border-b text-left text-xs uppercase tracking-wide text-slate-400"><th className="px-5 py-3">Semester</th><th>Semester dates</th><th>Grade deadline</th><th className="px-5 text-right">Actions</th></tr></thead><tbody>{(data?.items || []).map((term) => <tr key={term.id} className="border-b border-slate-100"><td className="px-5 py-3 font-semibold text-ink">{term.label}{term.is_active_planning_term && <span className="ml-2 rounded-full bg-brand-50 px-2 py-1 text-xs text-brand-700">Current</span>}</td><td className="text-slate-500">{term.start_date} — {term.end_date}</td><td><input type="date" className="field-input max-w-48" value={deadlines[term.id] ?? term.grade_submission_deadline ?? ""} onChange={(e) => setDeadlines((current) => ({ ...current, [term.id]: e.target.value }))} /></td><td className="px-5"><div className="flex justify-end gap-2"><button type="button" onClick={() => save(term)} disabled={saving === String(term.id)} className="btn-ghost cursor-pointer px-3 py-2"><Save className="h-4 w-4" /> Save</button><button type="button" onClick={() => remove(term)} disabled={term.is_active_planning_term || saving === `delete-${term.id}`} className="btn-ghost cursor-pointer px-3 py-2 text-red-600"><Trash2 className="h-4 w-4" /> Remove</button></div></td></tr>)}</tbody></table></div></Card>}
  </div>;
}
