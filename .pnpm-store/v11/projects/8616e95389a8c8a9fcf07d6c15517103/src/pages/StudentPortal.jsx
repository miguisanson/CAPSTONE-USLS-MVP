...
export default function StudentPortal() {
  ...
  const onboardingInProgress = Boolean(data?.onboarding_in_progress);

  useEffect(() => {
    if (onboardingInProgress && !ONBOARDING_NAV.some((item) => item.id === view)) setView("onboarding");
  }, [onboardingInProgress, view]);

  return (
    <div className="min-h-screen bg-canvas lg:flex">
      <RoleSidebar roleLabel="Student Portal" items={onboardingInProgress ? ONBOARDING_NAV : STUDENT_NAV} active={view} onChange={setView} />
          ...
          {loading ? (
            ...
            <>
              ...
              <StudentPortalSectionBoundary view={view}>
                {onboardingInProgress && view === "onboarding" && <StudentOnboardingPanel data={data} />}
                {onboardingInProgress && view === "profile" && <StudentProfilePanel student={student} />}
                ...
                {!onboardingInProgress && view === "lifecycle" && <div className="space-y-5"><ProgressPanel data={data} /><WorkflowStatusPanel data={data} /></div>}
                {!onboardingInProgress && view === "courses" && <MyCoursesPanel data={data} onSaved={refetch} />}
                {!onboardingInProgress && ["research", "schedule", "loa", "readmission", "awol", "practicum", "withdrawal", "graduation"].includes(view) && <RequestCenter data={data} onSaved={refetch} focusedRequest={view} />}
  ...
  return (
    <Card className="p-6">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        {data.onboarding_in_progress ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:w-[520px]"><MiniStat label="Stage" value="Onboarding" /><MiniStat label="Status" value="In process" badge /><MiniStat label="Program" value={student.program_code} /><MiniStat label="Account" value="Active" badge /></div> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:w-[520px]"><MiniStat label="Stage" value={student.current_stage} /><MiniStat label="Standing" value={student.standing} badge /><MiniStat label="Coursework" value={`${course_audit.completion_rate}%`} /><MiniStat label="Research" value={research_case?.status || "Not started"} badge /></div>}
      </div>
    </Card>
  );
}

function StudentOnboardingPanel({ data }) {
  return (
    <Card className="p-6">
      <SectionTitle title="Onboarding in process" subtitle="The Graduate School is reviewing your admission handoff" icon={UserCheck} />
      <div className="rounded-2xl border border-brand-200 bg-brand-50/60 p-5">
        <div className="flex items-start gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-brand-600 text-white"><Activity className="h-5 w-5" /></span><div><h3 className="font-semibold text-brand-900">Your onboarding is being processed</h3><p className="mt-1 max-w-2xl text-sm leading-relaxed text-brand-800">Your account is active. Graduate School staff are completing the required internal verification. You can review your profile and inbox while this is in progress.</p></div></div>
      </div>
      <div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Lifecycle stage</p><p className="mt-1 font-semibold text-ink">Onboarding</p></div><div className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Program</p><p className="mt-1 font-semibold text-ink">{data.student.program_code}</p></div><div className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Account notice</p><p className="mt-1 font-semibold text-ink">Recorded</p></div></div>
    </Card>
  );
}

function StudentProfilePanel({ student }) {
  const fields = [["Full name", student.name], ["Student number", student.student_number], ["Email", student.email], ["Program", `${student.program_code} — ${student.program_name}`], ["Entry year", student.entry_year], ["Standing", student.standing]];
  return <Card className="p-6"><SectionTitle title="Student profile" subtitle="Profile information currently recorded by the Graduate School" icon={Activity} /><dl className="grid gap-3 sm:grid-cols-2">{fields.map(([label, value]) => <div key={label} className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3"><dt className="text-xs font-bold uppercase tracking-wide text-slate-400">{label}</dt><dd className="mt-1 text-sm font-semibold text-slate-700">{value || "—"}</dd></div>)}</dl></Card>;
}
...
function MyCoursesPanel({ data, onSaved }) {
  ...
  return (
    <div className="space-y-5">
      {data.study_plan && <StudentStudyPlan plan={data.study_plan} onSaved={onSaved} />}
      <Card className="p-6">
...
function StudentStudyPlan({ plan, onSaved }) {
  const selectedExisting = (plan.recommendations || []).filter((item) => item.source === "Student Choice").map((item) => item.course_id);
  ...
  async function submitChoices() {
    ...
    try { const res = await api.submitCognateChoices({ case_id: plan.case_id, course_ids: selected }); setMessage(res.message); await onSaved(); }
    catch (err) { setError(err.message); } finally { setBusy(false); }
  }
  return <Card className="p-6"><div className="flex flex-wrap items-start justify-between gap-3"><SectionTitle title="Recommended subjects for the upcoming semester" subtitle={`${plan.target_term?.label || "Upcoming semester"} · This draft is visible while the Academic Coordinator reviews it`} icon={BookOpenCheck} /><StatusBadge value={plan.status} dot={false} /></div>{error && <p className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-sm font-semibold text-red-700">{error}</p>}{message && <p className="mt-3 rounded-xl bg-brand-50 px-3 py-2 text-sm font-semibold text-brand-800">{message}</p>}<div className="mt-4 grid gap-3 md:grid-cols-2">{(plan.recommendations || []).map((item) => <div key={item.id} className="rounded-xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-ink">{item.code} — {item.title}</p><p className="mt-1 text-xs text-slate-500">{item.units} units · {item.note}</p></div><StatusBadge value={item.source} dot={false} /></div></div>)}</div>{plan.can_choose_cognates && plan.optional_choices?.length > 0 && <div className="mt-6 border-t border-slate-100 pt-5"><h3 className="font-semibold text-ink">Choose optional cognates</h3><p className="mt-1 text-sm text-slate-500">Your choices become part of the same draft and are reviewed by the Academic Coordinator.</p><div className="mt-3 grid gap-2 md:grid-cols-2">{plan.optional_choices.map((item) => <label key={item.course_id} className="flex cursor-pointer gap-3 rounded-xl border border-slate-200 p-3"><input type="checkbox" className="mt-0.5 h-5 w-5 accent-brand-600" checked={selected.includes(item.course_id)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, item.course_id] : current.filter((id) => id !== item.course_id))} /><span><span className="block font-semibold text-ink">{item.code} — {item.title}</span><span className="text-xs text-slate-500">{item.units} units · {item.category}</span></span></label>)}</div><button type="button" className="btn-primary mt-4" onClick={submitChoices} disabled={busy || selected.length === 0}>{busy ? "Submitting…" : "Submit cognate choices"}</button></div>}</Card>;
}
