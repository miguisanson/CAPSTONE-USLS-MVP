...
const STEPS = ["Draft", "Proposed", "Submitted", "Approved", "Published"];
export default function CourseAdjustments() {
  ...
  const canEdit = (status === null || status === "Draft" || status === "Returned" || (status === "Proposed" && user?.role === "academic_coordinator")) && data?.planning_window_open !== false;
  ...
  async function saveDraft(showMessage = true) {
    ...
    try {
      const payload = {
        ...
        action: "draft",
        selections: buildSelections(data?.demand, sel),
      };
  ...
  async function planAction(action) {
    ...
    try {
      ...
      const res = await api.saveCourseAdjustmentPlan({ program_id: programId, term_label: termLabel, action, selections: action === "proposal" ? buildSelections(data?.demand, sel) : undefined });
  ...
  function updateSelection(row, field, value) {
    ...
    setSel((current) => ({
      ...current,
      [row.course.id]: { ...(current[row.course.id] || defaultSelection(row)), [field]: value },
    }));
    markDirty();
  }
          ...
          <div className="flex-1">
            ...
            <div className="mt-3 grid grid-cols-1 gap-2 text-xs text-slate-500 sm:grid-cols-2">
              <p>
                ...
                Research Coordinator / Academic Coordinator / GS Staff
              </p>
              <p>
          ...
          <Card className="p-5">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-50/70 p-4">
              ...
              <div className="flex flex-wrap gap-2">
                <ActionButton busy={busy === "draft"} onClick={() => planAction("draft")} icon={Settings2} disabled={!canEdit || user?.role === "staff"} primary={canEdit}>
                  {status === "Draft" || status === "Returned" ? "Update draft" : "Save draft"} ({offeredCount} offered)
                </ActionButton>
                {user?.role === "research_coordinator" && <ActionButton busy={busy === "proposal"} onClick={() => planAction("proposal")} icon={Send} disabled={!canEdit} primary={canEdit}>Send proposal to AC</ActionButton>}
                {user?.role === "academic_coordinator" && <ActionButton busy={busy === "submit"} onClick={() => planAction("submit")} icon={Send} disabled={!canEdit} primary={canEdit}>Submit to Dean</ActionButton>}
                <ActionButton busy={busy === "publish"} onClick={() => planAction("publish")} icon={CheckCircle2} disabled={status !== "Approved" || user?.role !== "staff"} primary={status === "Approved" && user?.role === "staff"}>
                  Publish
                </ActionButton>
              </div>
            </div>
                ...
                <table className="w-full min-w-[1380px] text-sm">
                  <thead>
                    <tr className="border-b border-slate-100 text-left text-xs font-bold uppercase tracking-wide text-slate-400">
                      ...
                      <th className="px-3 py-3">Qualified faculty</th>
                    ...
                    {data.demand.map((row) => {
                      ...
                      return (
                        <tr key={row.course.id} className="border-b border-slate-50">
                          ...
                          <td className="px-3 py-3"><select className="field-input min-w-52 py-1" value={s.facultyId} onChange={(event) => updateSelection(row, "facultyId", event.target.value)} disabled={!canEdit}><option value="">Select qualified faculty</option>{(row.qualified_faculty || []).map((faculty) => <option key={faculty.id} value={faculty.id}>{faculty.name}{faculty.preference_match ? " · Preferred" : ""} · {faculty.current_units}/24 units</option>)}</select></td>
                          <td className="px-3 py-3"><input className="field-input min-w-40 py-1" value={s.schedule} onChange={(event) => updateSelection(row, "schedule", event.target.value)} disabled={!canEdit} placeholder="e.g. Sat 9–12" /></td>
                          <td className="px-3 py-3"><input className="field-input min-w-52 py-1" value={s.rationale} onChange={(event) => updateSelection(row, "rationale", event.target.value)} disabled={!canEdit} placeholder="Adjustment rationale" /></td>
...
function defaultSelection(row) {
  ...
  return {
    ...
    rationale: row.adjustment_rationale || "",
  };
}

function buildSelections(rows = [], selections = {}) {
  return rows.map((row) => ({
    course_id: row.course.id,
    offer: !!selections[row.course.id]?.offer,
    section_count: Number(selections[row.course.id]?.sections) || row.suggested_sections,
    notes: selections[row.course.id]?.notes || "",
    faculty_id: selections[row.course.id]?.facultyId || undefined,
    schedule_label: selections[row.course.id]?.schedule || "",
    rationale: selections[row.course.id]?.rationale || "",
  }));
}

function planStatusNote(plan) {
  if (!plan || plan.status === "Draft") return "Draft saved. Review the demand list and submit when ready.";
  if (plan.status === "Proposed") return "Research Coordinator proposal sent. The Academic Coordinator can revise it and submit it to the Dean.";
