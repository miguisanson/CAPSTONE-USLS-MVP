...
export default function Faculty() {
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      ...
      {selected && <FacultyProfileModal faculty={selected} onClose={closeFaculty} onSaved={refetch} />}
    </div>
  );
}

function FacultyProfileModal({ faculty, onClose, onSaved }) {
      ...
      <Card className="max-h-[94vh] w-full max-w-7xl overflow-hidden shadow-2xl">
        ...
        <div className="max-h-[calc(94vh-104px)] space-y-6 overflow-y-auto p-4 sm:p-6">
          <div className="grid gap-6 lg:grid-cols-3">
            <TeachingProfileEditor faculty={faculty} onSaved={onSaved} />
...
function TeachingProfileEditor({ faculty, onSaved }) {
  const [phd, setPhd] = useState(Boolean(faculty.phd_holder));
  const [preferred, setPreferred] = useState((faculty.preferred_subjects || []).join(", "));
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  async function save() { setBusy(true); setMessage(""); try { const res = await api.updateFacultyTeachingProfile(faculty.id, { phd_holder: phd, preferred_subjects: preferred.split(",").map((item) => item.trim()).filter(Boolean) }); setMessage(res.message); await onSaved(); } catch (err) { setMessage(err.message); } finally { setBusy(false); } }
  return <ProfileSection icon={ShieldCheck} title="Teaching qualification"><label className="flex items-center gap-2 text-sm font-semibold text-ink"><input type="checkbox" checked={phd} onChange={(event) => setPhd(event.target.checked)} className="h-5 w-5 accent-brand-600" />PhD holder</label><p className="mt-3 text-xs font-bold uppercase tracking-wide text-slate-400">Allowable teaching load</p><p className="mt-1 text-sm font-semibold text-ink">24 units</p><label className="mt-3 block"><span className="text-xs font-bold uppercase tracking-wide text-slate-400">Preferred subject codes</span><textarea className="field-input mt-1" rows="3" value={preferred} onChange={(event) => setPreferred(event.target.value)} placeholder="MBA 201, MBA 204" /></label><button type="button" onClick={save} disabled={busy} className="btn-primary mt-3">{busy ? "Saving…" : "Save teaching profile"}</button>{message && <p className="mt-2 text-xs font-semibold text-slate-600">{message}</p>}</ProfileSection>;
}
