...
export const api = {
  ...
  saveMonitoringRecommendations: (payload) =>
    request("/monitoring/recommendations", { method: "POST", body: JSON.stringify(payload) }),
  ...
  onboardingCaseAction: (caseId, payload) =>
    request(`/onboarding/cases/${caseId}/action`, { method: "POST", body: JSON.stringify(payload) }),
  overwriteOnboardingStudent: (uploadId, payload) =>
    request(`/monitoring/uploads/${uploadId}/overwrite-student`, { method: "POST", body: JSON.stringify(payload) }),
  approveOnboardingBatch: (batchId) =>
    request(`/onboarding/batches/${batchId}/approve`, { method: "POST", body: JSON.stringify({}) }),
  curriculumPlanning: (params = {}) => {
    const actual = typeof params === "string" || typeof params === "number" ? { program_id: params } : params;
    const qs = queryString(actual);
    return request(`/curriculum-planning${qs ? `?${qs}` : ""}`);
  },
  semesterPlanning: (params = {}) => {
    const qs = queryString(params);
    return request(`/semester-planning${qs ? `?${qs}` : ""}`);
  },
  importCurriculumVersion: (file) => {
    const form = new FormData(); form.append("file", file);
    return requestForm("/semester-planning/curriculum/import", form);
  },
  previewSemesterEnrollment: (file, kind = "students") => {
    const form = new FormData(); form.append("file", file); form.append("kind", kind);
    return requestForm("/semester-planning/enrollment/preview", form);
  },
  applySemesterEnrollment: (payload) => request("/semester-planning/enrollment/apply", { method: "POST", body: JSON.stringify(payload) }),
  semesterStudentAction: (studentId, payload) => request(`/semester-planning/students/${studentId}/action`, { method: "POST", body: JSON.stringify(payload) }),
  semesterCaseAction: (caseId, payload) => request(`/semester-planning/case/${caseId}/action`, { method: "POST", body: JSON.stringify(payload) }),
  saveSemesterCompletion: (payload) => request("/semester-planning/completion", { method: "POST", body: JSON.stringify(payload) }),
  submitCognateChoices: (payload) => request("/student-portal/study-plan/cognates", { method: "POST", body: JSON.stringify(payload) }),
  acknowledgeCourseworkReport: (caseId) => request(`/semester-planning/case/${caseId}/acknowledge`, { method: "POST", body: JSON.stringify({}) }),
  updateFacultyTeachingProfile: (facultyId, payload) => request(`/faculty/${facultyId}/teaching-profile`, { method: "PATCH", body: JSON.stringify(payload) }),
  ...
  updateCurriculumOfferingFaculty: (id, payload) =>
    request(`/curriculum-planning/offerings/${id}/faculty`, { method: "PATCH", body: JSON.stringify(payload) }),
